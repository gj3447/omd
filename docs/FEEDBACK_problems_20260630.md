...
> 계기: 같은 날 BPC jg_bpc 가 **OMD 를 우회**해 공유 `kjra` 직접커밋 → 로컬 +17 divergence 발생.
...
**실증 (오늘 BPC)**:
- jg_bpc/prismv2 코드·워크플로에 OMD 호출 **0건**, OMD worktree 디렉토리(`*-omd-worktrees`/`*-omd-integration`) **0개** → 작업이 공유 dir 직행.
- `omd_mcp.db` = **6 task·Jun 28 에서 멈춤** → 오늘 daily 작업은 OMD 안 탐.
- 결과: 세션들이 공유 `kjra` 직접커밋 → 로컬 +17 미푸시 divergence(우리가 손으로 정리).
...
2. **우회 fail-loud** — 통합 브랜치 `pre-push`/CI 게이트가 *OMD merge-trailer(`CLOUD CONNECT <task>`) 없는* 커밋을 경고/거부. (오늘 우리가 jg_bpc `.git/hooks/post-commit` 으로 만든 divergence-nudge 가 이 빈약판.)
