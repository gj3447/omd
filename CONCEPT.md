...
하나의 구름에 통합한다. 기존 `kg-neo4j` / `logs-host`처럼 **백엔드 서버 1개 + MCP/CLI 얇은 클라이언트** 형태로
PI 워크스페이스에 통합한다.
...
omd cancel-wait <leaseId> <agent> <generation> --bail-epoch=N
omd rollover-claim <priorLeaseId> <agent> <generation> --bail-epoch=N --request-id=<opId>
omd release <leaseId>
omd renew   <leaseId>
...
`rollover-claim`은 종단 OrbitRequest의 outgoing transition이 아니다. 최신
`RELEASED`/`LEASE_EXPIRED`/`CANCEL`/`WAIT_TIMEOUT` predecessor를 정확히 fencing한 뒤
immutable intent로 별도 `N+1` 머신을 만든다. owner-reclaim과 row가 없는 `QUEUE_FULL`은
fresh semantic request ID를 사용하고, policy `DENIED`는 기존 자동 세대 증가 규칙을 유지한다.
동일 작업 영수증은 idempotency 보존 기간 안에서 재생되며, 이후에도 latest-predecessor
fence가 같은 predecessor의 둘째 successor를 막는다.
...
- `consumer-*` 서비스 패밀리에 **OMD Coordinator 서버** 추가, MCP + CLI 노출.
